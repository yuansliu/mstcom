...
#include "liblzma/Precomp.h"
#include "liblzma/CpuArch.h"
...
static SRes Encode(ISeqOutStream *outStream, ISeqInStream *inStream, UInt64 fileSize, char *rs, int _pb = 2)
{
  CLzmaEncHandle enc;
  SRes res;
  CLzmaEncProps props;
  ...
  props.level = 9, props.dictSize = 3 << 29, props.lc = 3, props.lp = props.pb = _pb, props.fb = 128;
...
public:
int lzma_main(int numArgs, char *args[], char *rs, int _pb = 2)
{
  ...
  if (encodeMode)
  {
    ...
    res = Encode(&outStream.vt, &inStream.vt, fileSize, rs, _pb);
  }
  else
  {
...
void lzma_compress(const char *infile, const char *outfile, int _pb) {
	lzma_class lzma;
	...
	rs[0] = '\0';
	int res = lzma.lzma_main(argv.size() - 1, argv.data(), rs, _pb);
	fputs(rs, stdout);
	delete[] rs;
	// return res;
}
...
void lzma_decompress(const char *infile, const char *outfile) {
	...
	char *rs = new char[1024];
	rs[0] = '\0';
	int res = lzma.lzma_main(argv.size() - 1, argv.data(), rs);
	fputs(rs, stdout);
	delete[] rs;
}
